...
call_count = 0  # track LLM calls per question
...
def call_llm(prompt, system="You are a helpful assistant.", temperature=0.7, max_tokens=1024):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            print(f"[call_llm] HTTP {resp.status_code}: {resp.text[:200]}")
            return None
    except Exception as e:
        print(f"[call_llm] Exception: {e}")
        return None

def extract_final_answer(text):
    """
    Try to pull out a clean final answer from model output.
    Looks for patterns like 'Answer: X' or '**X**' or just returns the last line.
    """
    if not text:
        return ""
